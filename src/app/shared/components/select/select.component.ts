...
import { isPlatformBrowser, NgTemplateOutlet } from '@angular/common'; // ⬅ tuyo: + NgTemplateOutlet
import {
  afterNextRender,
  booleanAttribute,
...
export class ZardSelectComponent implements ControlValueAccessor, OnDestroy {
  ...
  private getItemAlignedOffset(): { bottom: number; top: number } | null {
    ...
    const triggerHeight =
      trigger.offsetHeight || trigger.getBoundingClientRect().height || this.triggerHeight();
    const itemHeight =
